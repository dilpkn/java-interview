...
JVM (Java Virtual Machine) — это сердце Java-платформы. Это виртуальный компьютер, который выполняет Java-байт-код, обеспечивая главное обещание Java: «Write Once, Run Anywhere» (WORA).
...
JVM внутри себя превращает маленькие типы (boolean, byte, short, char) в обычные 32-битные числа (int), потому что так проще и быстрее работать.
byte и short сохраняют знак при расширении (отрицательные остаются отрицательными)
char расширяется нулями (так как не бывает отрицательным)
boolean внутри — это 0 (false) или 1 (true)
Все вычисления (сложение, умножение) происходят уже над int, а специальные команды есть только для загрузки и сохранения
...
Загрузчик - это механизм в Java, который находит и загружает классы в память, когда они нужны программе. JVM (система исполнения) не знает и не должна знать, откуда пришли классы. За это отвечает ClassLoader.
...
загружает ваши классы (classpath) и классы сторонних библиотек.

Загрузчик классов выполняет три основных действия в строгом порядке: 
* Загрузка: ClassLoader находит .class файл (или другой источник), читает байт-код и создает объект Class в памяти.
* Связывание: выполняет проверку, подготовку и (необязательно) разрешение. 
    - Проверка:JVM проверяет, что байт-код корректен и не делает ничего опасного.
    - Подготовка: выделяет память для переменных класса и инициализация памяти значениями по умолчанию. 
    - Разрешение: JVM заменяет символические ссылки на прямые ссылки. 
* Инициализация: JVM выполняет код инициализации:
